...
public class MaxArea11 {
    ...
    public static int maxArea2(int[] height) {
        //动态规划算法，每一轮最新的最大容积水受到上一轮最大值的影响

        //观察一下，你会发现之前的i的最大容积之后，如果i+1的容积比i大,那么i+1到他起始点的容积一定大过i
        int end = 1;
        int start = 0;
        ...
        while (end < height.length) {
            ...
            if ((end - start) * Math.min(height[start], height[end]) > maxAreas[end - 1]) {
                int tempArea = (end - start) * Math.min(height[start], height[end]);
                for (int i = 0; i < end; i++) {
                    if ((end - i) * Math.min(height[i], height[end]) > tempArea) {
                        tempArea = (end - i) * Math.min(height[i], height[end]);
                        start = i;
                    }
                }
                maxAreas[end] = tempArea;
            }
            end++;
        }
        int max = 0;
    ...
    public static int maxArea3(int[] height) {
        //官方解答 指针法，每次容积都受到最短的高度的影响,每次比较一次之后，移动最短的高度
