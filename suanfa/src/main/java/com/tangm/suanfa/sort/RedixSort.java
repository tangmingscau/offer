...
public class RedixSort {
    public static void main(String[] args) {
        ...
        System.out.println("基数排序耗时" + (System.currentTimeMillis() - start) + "\n");
     ...
     * a -- 数组
     */
    public static void radixSort(int[] toSorts) {
        int n = toSorts.length;
        int exp;    // 指数。当对数组按各位进行排序时，exp=1；按十位进行排序时，exp=10；...
        int max = getMax(toSorts, n);    // 数组a中的最大值

        // 从个位开始，对数组a按"指数"进行排序
        for (exp = 1; max / exp > 0; exp *= 10) {
            countSort(toSorts, n, exp);
        }
